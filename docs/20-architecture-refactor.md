...
- **G.14** ✅ Hot-fix three regressions surfaced after G.13 (characterisation only covers happy paths):
  - **G.14a** Runner now wraps `strategy.adaptJson()` in try/catch so a downstream throw (e.g. naked `JSON.parse` on tool-call args) still emits `request_end` per the `db/request-sink` contract; rethrown so route-level `forwardError` runs.
  - **G.14b** `RequestContext.stream` added; Runner reads it for the dispatch-reject error log instead of hardcoding `stream: false`. All five route-side `RunnerCtx` constructions populate it from `payload.stream`.
  - **G.14c** `copilotOpenAIDirectShim.describeEndLog` gained an `error` arm returning `{ model: result.req.model }` so the DB sink + terminal stop rendering "unknown" when Copilot dispatch fails.
...
- **H.19** ✅ **Activate dep-cruiser rule #3 (partial)**: `core/` may not import `strategies/` or `upstream/`; `strategies/*.ts` may not import `infra/state` or `lib/state`. The full `routes/` ↛ `strategies|upstream` lock is deferred to **J.7** because route handlers still import upstream payload types (`ChatCompletionsPayload`, `ResponsesPayload`, etc.) directly until those move into `protocols/` or get re-exported through `composition/`.
...
- **J.4** ✅ Anti-ban rules in place in `test/e2e/` (1 request per test, fail-fast). Verified per CLAUDE.md E2E protocol; script `bun run test:e2e` preserved.
- **J.5** ✅ `test/e2e/refactor/` retained as the opt-in `bun run test:e2e:full` target (`RAVEN_E2E_FULL=1`). Script present in `package.json`.
...
- **J.8** ✅ Final Phase C run completed 2026-04-23 against the post-refactor proxy on a sandbox port (17024) to avoid clashing with any locally running prod proxy. Result: **41 pass / 0 fail / 27 filtered-out** across 5 e2e files (`proxy.e2e.test.ts`, `native-anthropic.e2e.test.ts`, `openai-completions.e2e.test.ts`, `context-1m.e2e.test.ts`) in 155.43s — covers all six strategies (copilot-native, copilot-translated, copilot-openai-direct, copilot-responses, custom-openai, custom-anthropic) across stream/non-stream, tool-use, server-side `web_search`, model-name translation, thinking, 1M-context, parallel tool calls. No-diff against the Phase C baseline.
