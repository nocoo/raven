...
```bash
websocat "ws://localhost:7024/ws/logs?token=rk-xxx&level=debug"
```
...
| 8 | `test: add unit tests for WS log handler` ✅ | `test/ws/logs.test.ts` |
...
3. **WebSocket 验证**：`websocat ws://localhost:7024/ws/logs` → 看实时事件流
