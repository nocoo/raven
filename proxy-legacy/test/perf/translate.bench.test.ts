...
import { translateResponse } from "../../src/translate/openai-to-anthropic.ts";
import { createStreamTranslator } from "../../src/translate/stream.ts";
import type {
  AnthropicRequest,
  OpenAIResponse,
  OpenAIStreamChunk,
...
function makeComplexRequest(): AnthropicRequest {
  return {
...
function makeComplexResponse(): OpenAIResponse {
  return {
    id: "chatcmpl-perf",
...
function makeStreamChunks(): OpenAIStreamChunk[] {
...
describe("translate performance benchmarks", () => {
  test(`anthropic-to-openai request translation < ${REQUEST_THRESHOLD_MS}ms (avg over ${ITERATIONS} iterations)`, () => {
    ...
    // Warmup
    for (let i = 0; i < 10; i++) translateRequest(request);
  ...
  test(`openai-to-anthropic response translation < ${RESPONSE_THRESHOLD_MS}ms (avg over ${ITERATIONS} iterations)`, () => {
    ...
    // Warmup
    for (let i = 0; i < 10; i++) translateResponse(response);
  ...
  test(`stream state machine < ${STREAM_THRESHOLD_MS}ms/chunk (${STREAM_CHUNKS} chunks)`, () => {
    ...
    // Warmup
    for (let i = 0; i < 3; i++) {
      const t = createStreamTranslator("warmup", "model");
      for (const chunk of chunks) t.processChunk(chunk);
    }

    const start = performance.now();
    for (let iter = 0; iter < 10; iter++) {
      const translator = createStreamTranslator("perf-test", "model");
