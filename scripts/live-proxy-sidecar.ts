...
const { values } = parseArgs({ args: process.argv.slice(2), options: { execute: { type: "boolean", default: false }, limit: { type: "string", default: "66" } } })
if (!values.execute) {
  ...
  try {
    ...
    console.error("诊断 Proxy 启动或运行失败；未重试。请检查已有配置、凭证和日常 Proxy。")
    process.exitCode = 1
  }
}
...
async function run() {
  ...
  assert.ok(config.apiKey && config.internalKey, "Live sidecar requires the existing API and management environment keys")
  ...
  const db = new Database(config.dbPath, { create: false })
  let stopSink: (() => void) | undefined
  let server: ReturnType<typeof Bun.serve> | undefined
  const originalFetch = globalThis.fetch
  try {
    disableTerminalSink()
    const response = await originalFetch(`http://127.0.0.1:${config.port}/api/settings`, { headers: { Authorization: `Bearer ${config.internalKey}` }, redirect: "error", signal: AbortSignal.timeout(10_000) })
    ...
    const app = createApp({ db, apiKey: config.apiKey, internalKey: config.internalKey, githubToken: state.githubToken })
