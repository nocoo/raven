...
export function useUpstreams(initial: ProviderPublic[], offset: number, now: number) {
  ...
  const run = async (kind: string, title: string, action: () => Promise<void>) => {
    if (locked.current) return;
    locked.current = true; setBusy(kind); setFeedback(null);
    try { await action(); }
    catch (cause) { setFeedback({ kind: "error", title, cause }); }
  ...
  const save = () => run("save", "Upstream could not be saved", async () => {
    ...
    adopt(saved); setFeedback({ kind: "success", message: "Upstream saved." });
  });
  const remove = () => run("delete", "Upstream could not be deleted", async () => {
    ...
    setUpstreams(remaining); load(remaining[0] ?? null); setFeedback({ kind: "success", message: "Upstream deleted." });
  });
  const refresh = () => run("refresh", "Model refresh failed", async () => {
    ...
    setFeedback({ kind: "success", message: "Models refreshed. Manual IDs were preserved." });
  });
  const reload = () => run("status", "Quota status could not be loaded", async () => {
    ...
    setFeedback({ kind: "success", message: "Quota status updated." });
  });
  const test = () => run("test", "Model test failed", async () => {
    ...
    setFeedback({ kind: "diagnostic", result });
  });
