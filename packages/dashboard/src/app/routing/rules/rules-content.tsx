...
function RulesWorkbench({ rules, upstreams, clock }: { rules: RoutingRule[]; upstreams: ProviderPublic[]; clock: EditorClock }) {
  ...
  return <div className="space-y-4">
    <PageHeader title="Routing Rules" description="Choose an upstream and model, then add a schedule if you need one." actions={<Button size="sm" onClick={() => void choose(null)} disabled={state.busy || !state.active}><Plus className="size-4" />New rule</Button>} />
    <div className="routing-workbench">
      <LayerCard padding="sm" className="routing-directory space-y-1">
        ...
        <p className="px-2 pt-3 text-xs leading-relaxed text-basalt-muted-foreground">Assign a rule to your client keys in <a href="/connect" className="text-basalt-primary underline underline-offset-2">Connect</a>.</p>
      </LayerCard>
      <fieldset className="min-w-0 routing-enter" key={state.active?.id ?? "new"} disabled={state.busy}><legend className="sr-only">Rule editor</legend>
        <LayerCard role="region" aria-label="Rule configuration">
          ...
          <LayerCard.Body className="space-y-4">
            ...
            <Tabs value={tab} onValueChange={setTab} className="min-w-0">
              ...
              <TabsContent value="targets" className="space-y-4">
                <div className="space-y-1"><h3 className="text-sm font-semibold">Default chain</h3><p className="text-sm text-basalt-muted-foreground">{state.draft.mode === "all_day" ? "Used all day." : "Used outside your scheduled periods."} Requests for <code>auto</code> use the model below. An explicit client model keeps its name on the selected upstream.</p></div>
              ...
              <TabsContent value="schedule" className="space-y-4">
                <div className="space-y-1"><h3 className="text-sm font-semibold">Time-period overrides</h3><p className="text-sm text-basalt-muted-foreground">Use a different chain during selected hours. The default chain covers every gap.</p></div>
              ...
              <TabsContent value="protocol" className="space-y-4">
                <div className="flex items-start justify-between gap-4"><div className="space-y-1"><label htmlFor="allow-conversion" className="text-sm font-medium">Allow protocol conversion</label><p className="text-sm text-basalt-muted-foreground">Native protocols are recommended. Enable this only when your client uses a different API format from the chosen upstream.</p></div><Switch id="allow-conversion" checked={state.draft.allow_conversion} onCheckedChange={allow_conversion => state.change({ ...state.draft, allow_conversion })} /></div>
                <LayerCard.Well className="space-y-2 text-sm text-basalt-muted-foreground"><p>This setting applies to the default chain and every scheduled period.</p><p>Review supported formats in each chain’s protocol preview. A failed request stays on its selected upstream; only an exhausted quota advances to the next target.</p></LayerCard.Well>
              </TabsContent>
            </Tabs>
          </LayerCard.Body>
        </LayerCard>
