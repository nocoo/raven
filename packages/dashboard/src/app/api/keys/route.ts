...
import type { ApiKeyPublic, ApiKeyCreated } from "@/lib/types";

export const dynamic = "force-dynamic";

export async function GET() {
  try {
    const data = await proxyFetch<ApiKeyPublic[]>("/api/keys");
...
export async function POST(request: Request) {
  try {
    ...
    const data = await proxyFetch<ApiKeyCreated>("/api/keys", {
      method: "POST",
