...
export function IPDistribution({ data }: { data: MonitorData }) {
  ...
  return <MonitorPanel title="Source IPs" description="Request distribution and activity in the selected time range. Includes denied attempts." action={<div className="flex flex-wrap gap-2">{editable && <IPPolicyDialog keyId={keyId} />}<IPPolicyDialog /></div>}>
    <div className="grid gap-4 @min-[45rem]/panel:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
      ...
      <div className="min-w-0"><AreaChart data={activity.points} series={activity.series.map((item, index) => ({ key: item.key, label: item.label, color: CHART_COLORS[index % CHART_COLORS.length]! }))} stacked className="h-64" ariaLabel="Source IP requests over time" xValueFormatter={value => formatAxisTime(Number(value), data.window.to - data.window.from)} valueFormatter={formatCompact} summary="Top five source IPs over time; remaining IPs are grouped as Others. Select an IP in the distribution to inspect its requests." /></div>
    </div>
