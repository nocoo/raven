...
export function QuotaStatusView({ upstream }: { upstream: ProviderPublic }) {
  ...
  return <LayerCard padding="sm" className="space-y-3">
    <div className="flex flex-wrap items-center gap-2"><Gauge className="size-4 text-basalt-primary" /><h3 className="text-sm font-medium">Shared quota</h3><Badge variant={exhausted ? "warning" : limited ? "info" : "secondary"} className="text-xs">{exhausted ? "Exhausted" : limited ? "Soft limit" : "No limit"}</Badge><Badge variant={status.healthy ? "success" : "destructive"} className="ml-auto text-xs">{status.healthy ? "Accounting healthy" : "Accounting blocked"}</Badge></div>
...
export function QuotaEditor({ value, onChange, clock }: { value: QuotaDraft; onChange: (value: QuotaDraft) => void; clock: EditorClock }) {
  ...
  return <div className="space-y-4">
    ...
    </> : <LayerCard.Well className="py-3 text-sm text-basalt-muted-foreground">This upstream is always eligible by quota. If it is a quota candidate, later targets are unreachable.</LayerCard.Well>}
  </div>;
}
