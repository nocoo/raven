...
function SummarySkeleton() {
  return <div className="grid grid-cols-1 gap-3 @min-[22rem]/page:grid-cols-2 @min-[60rem]/page:grid-cols-4">
...
function PanelSkeleton({ children }: { children: ReactNode }) {
  return <LayerCard padding="none" className="@container/panel min-w-0"><LayerCard.Header><div className="w-full space-y-2"><Skeleton className="h-4 w-36" /><Skeleton className="h-3 w-3/4" /></div></LayerCard.Header><LayerCard.Well className="space-y-4 py-3">{children}</LayerCard.Well></LayerCard>;
}
...
function FormSkeleton() {
  return <PanelSkeleton><div className="grid gap-4 @min-[26rem]/panel:grid-cols-2">{ROWS.slice(0, 4).map(key => <div key={key} className="space-y-2"><Skeleton className="h-3 w-24" /><Skeleton className="h-9 w-full" /></div>)}</div><Skeleton className="h-8 w-24" /></PanelSkeleton>;
}

export function OverviewLoading() {
  return <LoadingPage title="Overview" filters><SummarySkeleton /><div className="grid gap-3 @min-[60rem]/page:grid-cols-[minmax(0,1.8fr)_minmax(0,1fr)]"><ChartSkeleton /><PanelSkeleton><RowsSkeleton count={3} /></PanelSkeleton></div><div className="grid gap-3 @min-[48rem]/page:grid-cols-2"><PanelSkeleton><RowsSkeleton /></PanelSkeleton><PanelSkeleton><RowsSkeleton /></PanelSkeleton></div></LoadingPage>;
}
...
export function UsageStatsSkeleton({ selected = false }: { selected?: boolean }) {
  return <div role="status" aria-label="Loading usage statistics" aria-busy="true">
    <div className="space-y-4" aria-hidden="true">
      <SummarySkeleton />
      <div className="grid gap-3 @min-[60rem]/page:grid-cols-[minmax(0,1fr)_minmax(0,3fr)]">
        ...
        <PanelSkeleton><div className="flex justify-between gap-3"><Skeleton className="h-4 w-36" /><Skeleton className="h-4 w-48" /></div><Skeleton className="h-3 w-2/3" /><div className="grid gap-4 @min-[34rem]/panel:grid-cols-[minmax(0,3fr)_minmax(0,2fr)]"><RowsSkeleton /><RowsSkeleton count={3} /></div></PanelSkeleton>
      </div>
      <ChartSkeleton />
      <div className="grid gap-3 @min-[60rem]/page:grid-cols-2"><ChartSkeleton /><PanelSkeleton><RowsSkeleton count={3} /></PanelSkeleton></div>
