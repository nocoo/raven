...
import type { OpenAIResponse } from "../../src/translate/types.ts";
...
describe("text response", () => {
  test("text content → text block", () => {
    const result = translateResponse(makeResponse());
...
describe("stop_reason mapping", () => {
  ...
  test("null finish_reason → null stop_reason", () => {
    const result = translateResponse(
      makeResponse({
        choices: [
          {
...
describe("usage mapping", () => {
  ...
  test("E3: missing usage → defaults to zeros", () => {
    ...
    // @ts-expect-error - testing missing usage
