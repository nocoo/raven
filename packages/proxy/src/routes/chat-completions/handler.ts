...
export async function handleCompletion(c: Context) {
  ...
  try {
    if (selectedModel) {
      ...
      logger.info(`Current token count: ${JSON.stringify(tokenCount)}`)
    } else {
      logger.warn("No model selected, skipping token count calculation")
    }
  } catch (error) {
    logger.warn("Failed to calculate token count", { error: String(error) })
  }
  ...
  if (isNonStreaming(response)) {
    logger.debug(`Non-streaming response: ${JSON.stringify(response).slice(-400)}`)
